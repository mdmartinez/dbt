...
class CleanTask(BaseTask):
    def __is_project_path(self, path):
        ...
        return (
            not os.path.commonprefix([proj_path, os.path.abspath(path)])
            == proj_path
        )
    ...
    def __is_protected_path(self, path):
        ...
        protected_paths = (
            self.project["source-paths"] + self.project["test-paths"] + ["."]
        )

        protected_abs_paths = [os.path.abspath for p in protected_paths]
        return abs_path in set(protected_abs_paths) or self.__is_project_path(
            abs_path
        )
