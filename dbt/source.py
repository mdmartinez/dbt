...
class Source(object):
    def __init__(self, project, own_project=None):
        ...
        self.own_project = (
            own_project if own_project is not None else self.project
        )
    ...
    def build_models_from_file_matches(
        self, to_build, file_matches, extra_args=[]
    ):
        ...
        build_args = [
            [
                ...
                file_match.get('relative_path'), self.own_project
            ] + extra_args for file_match in file_matches
        ]
    ...
    def get_csvs(self, csv_dirs):
        file_matches = dbt.clients.system.find_matching(
            self.own_project_root, csv_dirs, "[!.#~]*.csv"
        )

        return self.build_models_from_file_matches(Csv, file_matches)
