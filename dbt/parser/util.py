...
class ParserUtils(object):
    @classmethod
    def resolve_ref(
        cls, flat_graph, target_model_name, target_model_package,
        ...
        target_model = dbt.utils.find_refable_by_name(
            flat_graph, target_model_name, current_project
        )
        ...
        target_model = dbt.utils.find_refable_by_name(
            flat_graph, target_model_name, node_package
        )
        ...
        return dbt.utils.find_refable_by_name(
            flat_graph, target_model_name, None
        )
        ...
        for _, node in manifest.nodes.items():
            ...
            for ref in node.get('refs', []):
                ...
                target_model = cls.resolve_ref(
                    flat_graph, target_model_name, target_model_package,
                    current_project, node.get('package_name')
                )
