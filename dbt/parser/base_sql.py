    ...
    def load_and_parse(
        ...
        for file_match in file_matches:
            ...
            original_file_path = os.path.join(
                file_match.get("searched_path"), path
            )
        ...
        return cls.parse_sql_nodes(
            result, root_project, all_projects, tags, macros
        )

    @classmethod
    def parse_sql_nodes(
        cls, nodes, root_project, projects, tags=None, macros=None
    ):
        ...
        for n in nodes:
            ...
            node_path = cls.get_path(
                node.get("resource_type"), package_name, node.get("name")
            )

            node_parsed = cls.parse_node(
                node,
                node_path,
                root_project,
