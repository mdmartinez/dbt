...
class DefaultAdapter(object):
    ...
    raw_functions = [
        "get_status", "get_result_from_cursor", "quote", "convert_type"
    ]
            ...
            if target_column is not None and \
               target_column.can_expand_to(reference_column):
                ...
                logger.debug(
                    ...
                    target_column.data_type, new_type, to_schema, to_table
                )

                cls.alter_column_type(
                    profile, project_cfg, to_schema, to_table, column_name,
                    new_type, model_name
                )
    ...
    def acquire_connection(cls, profile, name):
        ...
        try:
            ...
            logger.debug(
                'Opening a new connection ({} currently allocated)'
    ...
    def add_query(
        ...
        with cls.exception_handler(profile, sql, model_name, connection_name):
            ...
            logger.debug(
                "SQL status: %s in %0.2f seconds", cls.get_status(cursor),
                (time.time() - pre)
            )
